...
public final class BuiltinAttributes {
    ...
    public static final ItemAttribute LIFESTEAL = register(ItemAttribute.builder()
            .id("lifesteal")
            .name("Lifesteal")
            .type(AttributeType.INTEGER)
            .description("Adds lifesteal to the player.")
            .display(Component.text("%operator%%value%% Lifesteal", NamedTextColor.LIGHT_PURPLE))
            .color(NamedTextColor.LIGHT_PURPLE)
            .slots(List.of(SlotGroup.MAIN_HAND, SlotGroup.OFF_HAND))
            .attribute(AttributeApplicator.builder()
                    .type(AttributeBridgeType.REDIRECT)
                    .key(NamespacedKey.fromString("itemattributes:lifesteal"))
                    .sourceConfig(AttributeBridgeConfig.builder()
                            .type(AttributeBridgeType.BUILTIN)
                            .key(NamespacedKey.fromString("itemattributes:damage"))
                            .operation(AttributeOperation.ADD)
                            .modifierExpression("%value%/100 * %damage%")
                            .event("entity_damage")
                            .build()
                    )
                    .destinationConfig(AttributeBridgeConfig.builder()
                            .type(AttributeBridgeType.VANILLA)
                            .key(NamespacedKey.minecraft("health"))
                            .operation(AttributeOperation.ADD)
                            .build()
                    )
            )
